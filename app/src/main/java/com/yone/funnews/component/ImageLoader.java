...
public class ImageLoader {
    ...
    public static void load(Context context, String url, BlurTransformation blurTransformation,ImageView iv){
        if (!SharedPreferenceUtil.getNoImageState()){
            Glide.with(context).load(url).bitmapTransform(blurTransformation).crossFade().diskCacheStrategy(DiskCacheStrategy.SOURCE).into(iv);
        }
    }
