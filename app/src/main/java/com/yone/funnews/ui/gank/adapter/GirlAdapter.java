...
public class GirlAdapter extends RecyclerView.Adapter<GirlAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        //存在记录的高度时先Layout在异步加载图片
        if (mList.get(holder.getAdapterPosition()).getHeight() > 0){
            ViewGroup.LayoutParams layoutParams = holder.mIvGirl.getLayoutParams();
                    ...
                    public void onResourceReady(Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                        if (holder.getAdapterPosition() != RecyclerView.NO_POSITION) {
                            if (mList.get(holder.getAdapterPosition()).getHeight() <= 0) {
                                ...
                                ViewGroup.LayoutParams lp = holder.mIvGirl.getLayoutParams();
                                lp.height = realHeight;
                            }
                            holder.mIvGirl.setImageBitmap(resource);
                        }
                    }
                });
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (onItemClickListener != null) {
                    View shreView = view.findViewById(R.id.iv_girl);
                    onItemClickListener.onItemClickListener(holder.getAdapterPosition(),shreView);
                }
            }
        });
    }
